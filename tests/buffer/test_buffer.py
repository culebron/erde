...
def test_geoseries():
	...
	res = 5
	bufs2_df = buffer(points_df, 500, resolution=res)
	...
	bufs4_df = buffer(points_df, 500, resolution=5, dissolve=True)
	...
	bufs5_df = buffer(p2, 500, resolution=5, default_crs=4326)
	...
	with pytest.raises(TypeError):
		buffer(points_df.geometry.values[0], 500)
	...
	def test_styles(c, j):
		with mock.patch('geopandas.base.GeoPandasBase.buffer', new=mock.MagicMock(return_value=bufs_df[:1].geometry)) as mck:
			buffer(points_df[:1], 750, cap_style=c, join_style=j)
